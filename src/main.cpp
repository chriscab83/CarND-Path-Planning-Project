...
double are_same(double a, double b) {
...
// trajectory
// ---------------------------------------------------------------------
            // the (x,y) points to be used by the path planner.
            vector<double> next_x_vals;
            vector<double> next_y_vals;

            // add left over previous path points to path planner.
            for (int i = 0; i < previous_path_x.size(); ++i) {
              next_x_vals.push_back(previous_path_x[i]);
              next_y_vals.push_back(previous_path_y[i]);
            }

            // points to use in spline for path planning
            vector<double> ptsx;
            vector<double> ptsy;
            ...
            // if not much previous data, extrapolate previous two car positions if traveling straight.
            if (prev_size < 2) {
            ...
            // otherwise, push in the last two points of previous path.
            else {
            ...
            // shift starting reference angle to 0 degrees.
            ...
            // calculate how to break up spline points so that we travel at the desired velocity
            ...
            for (int i = 1; i < 50-previous_path_x.size(); ++i) {
              ...
              // rotate back to normal reference angle
              x_point = x_ref*cos(ref_yaw) - y_ref*sin(ref_yaw);
              y_point = x_ref*sin(ref_yaw) + y_ref*cos(ref_yaw);
              x_point += ref_x;
              y_point += ref_y;

              // add points to path xy
